module.exports = {
  name: 'raiding',
  description: 'Configure la protection contre les raids',
  async execute(message, args, client) {
    ...
    try {
      ...
      if (!args[0]) {
        const isEnabled = serverConfig.antiraid.raiding || false;
        return message.reply({
          embeds: [{
            title: '🛡️ Protection contre les raids',
            description: `La protection est actuellement **${isEnabled ? 'activée' : 'désactivée'}**.`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: 'ℹ️ Information',
                value: 'Cette protection détecte et bloque les tentatives de raid sur le serveur.',
                inline: false
              },
              {
                name: '⚙️ Configuration',
                value: 'Utilisez `raiding on` pour activer ou `raiding off` pour désactiver.',
                inline: false
              }
            ],
            timestamp: new Date()
      ...
      serverConfig.antiraid.raiding = isEnabled;

      message.reply({
        embeds: [{
          title: isEnabled ? '✅ Protection activée' : '❌ Protection désactivée',
          description: `La protection contre les raids a été ${isEnabled ? 'activée' : 'désactivée'}.`,
          color: isEnabled ? 0x00ff00 : 0xff0000,
          fields: [
            {
              name: '🛡️ État',
      ...
      if (serverConfig.logChannel) {
        ...
        if (logChannel) {
          const logEmbed = {
            title: '⚙️ Protection anti-raid modifiée',
            description: `La protection contre les raids a été ${isEnabled ? 'activée' : 'désactivée'} par ${message.author.tag}`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: '👮 Modérateur',
      ...
      console.error('Erreur raiding:', error);
      message.reply({
        embeds: [{
          title: '❌ Erreur',
          description: 'Une erreur est survenue lors de la configuration de la protection contre les raids.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
