module.exports = {
  name: 'antiban',
  description: 'Configure la protection contre les bannissements massifs',
  async execute(message, args, client) {
    ...
    try {
      ...
      if (!args[0]) {
        const isEnabled = serverConfig.antiraid.antiBan || false;
        return message.reply({
          embeds: [{
            title: '🛡️ Protection contre les bannissements',
            description: `La protection est actuellement **${isEnabled ? 'activée' : 'désactivée'}**.`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: 'ℹ️ Information',
                value: 'Cette protection détecte et empêche les bannissements massifs de membres.',
                inline: false
              },
              {
                name: '⚙️ Configuration',
                value: 'Utilisez `antiban on` pour activer ou `antiban off` pour désactiver.',
                inline: false
              }
            ],
            timestamp: new Date()
      ...
      serverConfig.antiraid.antiBan = isEnabled;

      message.reply({
        embeds: [{
          title: isEnabled ? '✅ Protection activée' : '❌ Protection désactivée',
          description: `La protection contre les bannissements massifs a été ${isEnabled ? 'activée' : 'désactivée'}.`,
          color: isEnabled ? 0x00ff00 : 0xff0000,
          fields: [
            {
              name: '🛡️ État',
      ...
      if (serverConfig.logChannel) {
        ...
        if (logChannel) {
          const logEmbed = {
            title: '⚙️ Protection anti-ban modifiée',
            description: `La protection contre les bannissements massifs a été ${isEnabled ? 'activée' : 'désactivée'} par ${message.author.tag}`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: '👮 Modérateur',
      ...
      console.error('Erreur antiban:', error);
      message.reply({
        embeds: [{
          title: '❌ Erreur',
          description: 'Une erreur est survenue lors de la configuration de la protection contre les bannissements.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
