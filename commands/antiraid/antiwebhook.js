module.exports = {
  name: 'antiwebhook',
  description: 'Configure la protection contre la création de webhooks',
  async execute(message, args, client) {
    ...
    try {
      ...
      if (!args[0]) {
        const isEnabled = serverConfig.antiraid.antiWebhook || false;
        return message.reply({
          embeds: [{
            title: '🛡️ Protection contre les webhooks',
            description: `La protection est actuellement **${isEnabled ? 'activée' : 'désactivée'}**.`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: 'ℹ️ Information',
                value: 'Cette protection empêche la création de webhooks par les membres non autorisés.',
                inline: false
              },
              {
                name: '⚙️ Configuration',
                value: 'Utilisez `antiwebhook on` pour activer ou `antiwebhook off` pour désactiver.',
                inline: false
              }
            ],
            timestamp: new Date()
      ...
      serverConfig.antiraid.antiWebhook = isEnabled;

      message.reply({
        embeds: [{
          title: isEnabled ? '✅ Protection activée' : '❌ Protection désactivée',
          description: `La protection contre la création de webhooks a été ${isEnabled ? 'activée' : 'désactivée'}.`,
          color: isEnabled ? 0x00ff00 : 0xff0000,
          fields: [
            {
              name: '🛡️ État',
      ...
      if (serverConfig.logChannel) {
        ...
        if (logChannel) {
          const logEmbed = {
            title: '⚙️ Protection anti-webhook modifiée',
            description: `La protection contre la création de webhooks a été ${isEnabled ? 'activée' : 'désactivée'} par ${message.author.tag}`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: '👮 Modérateur',
      ...
      message.reply({
        embeds: [{
          title: '❌ Erreur',
          description: 'Une erreur est survenue lors de la configuration de la protection contre les webhooks.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
