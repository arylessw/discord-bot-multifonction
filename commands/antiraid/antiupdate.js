module.exports = {
  name: 'antiupdate',
  description: 'Configure la protection contre les mises à jour de serveur',
  async execute(message, args, client) {
    ...
    try {
      ...
      if (!args[0]) {
        const isEnabled = serverConfig.antiraid.antiUpdate || false;
        return message.reply({
          embeds: [{
            title: '🛡️ Protection contre les mises à jour de serveur',
            description: `La protection est actuellement **${isEnabled ? 'activée' : 'désactivée'}**.`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: 'ℹ️ Information',
                value: 'Cette protection empêche les modifications non autorisées des paramètres du serveur.',
                inline: false
              },
              {
                name: '⚙️ Configuration',
                value: 'Utilisez `antiupdate on` pour activer ou `antiupdate off` pour désactiver.',
                inline: false
              }
            ],
            timestamp: new Date()
      ...
      serverConfig.antiraid.antiUpdate = isEnabled;

      message.reply({
        embeds: [{
          title: isEnabled ? '✅ Protection activée' : '❌ Protection désactivée',
          description: `La protection contre les mises à jour de serveur a été ${isEnabled ? 'activée' : 'désactivée'}.`,
          color: isEnabled ? 0x00ff00 : 0xff0000,
          fields: [
            {
              name: '🛡️ État',
      ...
      if (serverConfig.logChannel) {
        ...
        if (logChannel) {
          const logEmbed = {
            title: '⚙️ Protection anti-mise à jour modifiée',
            description: `La protection contre les mises à jour de serveur a été ${isEnabled ? 'activée' : 'désactivée'} par ${message.author.tag}`,
            color: isEnabled ? 0x00ff00 : 0xff0000,
            fields: [
              {
                name: '👮 Modérateur',
      ...
      console.error('Erreur antiupdate:', error);
      message.reply({
        embeds: [{
          title: '❌ Erreur',
          description: 'Une erreur est survenue lors de la configuration de la protection contre les mises à jour de serveur.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
