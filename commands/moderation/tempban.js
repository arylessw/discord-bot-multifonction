...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!target.bannable) {
      return message.reply({
        embeds: [{
          ...
          description: 'Je ne peux pas bannir cet utilisateur. Vérifiez que mon rôle est plus haut que celui de l\'utilisateur.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
    ...
    try {
      ...
      await target.ban({ reason: `${reason} (Tempban: ${duration})` });

      // Envoyer un message de confirmation
      message.reply({
        embeds: [{
          title: '🔨 Bannissement temporaire',
          description: `${target.user.tag} a été banni temporairement.`,
          color: 0xff0000,
          thumbnail: {
      ...
      try {
        await target.send({
          embeds: [{
            title: '🔨 Bannissement temporaire',
            description: `Vous avez été banni temporairement de ${message.guild.name}.`,
            color: 0xff0000,
            fields: [
              {
                name: '👤 Modérateur',
      ...
      setTimeout(async () => {
        try {
          await message.guild.members.unban(target.id, 'Fin du bannissement temporaire');
            ...
            if (logChannel) {
              logChannel.send({
                embeds: [{
                  ...
                  description: `${target.user.tag} a été débanni automatiquement.`,
                  color: 0x00ff00,
                  thumbnail: {
                    url: target.user.displayAvatarURL({ dynamic: true })
                  },
                  fields: [
                    {
                      name: '📝 Raison',
                      value: 'Fin du bannissement temporaire',
                      inline: true
                    },
                    {
                      name: '🆔 ID de la sanction',
          ...
          console.error('Erreur lors du débannissement automatique:', error);
        }
      }, durationMs);
    } catch (error) {
      ...
      message.reply({
        embeds: [{
          title: '❌ Erreur',
          description: 'Une erreur est survenue lors du bannissement temporaire de l\'utilisateur.',
          color: 0xff0000,
          timestamp: new Date()
        }]
      });
